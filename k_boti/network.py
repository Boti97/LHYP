...
def train_neural_network(neural_inputs_pickles_path):
    ...
    train_x = concat_all_data_x[:int(len(concat_all_data_x) * 0.8)].type(torch.float32)
    train_y = all_data_y[:int(len(all_data_y) * 0.8)].type(torch.int64)
    ...
    all__train_idx = np.arange(len(train_x))
